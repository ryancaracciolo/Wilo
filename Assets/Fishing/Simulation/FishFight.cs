...
public class FishFight
{
    ...
    {
        size = Mathf.InverseLerp(0.5f, 12f, Mathf.Clamp(pounds, 0.5f, 12f));
        BarHeight = Mathf.Lerp(0.24f, 0.14f, size);
        fishY = 0.52f;
        fishTarget = 0.52f;
        barY = 0.38f;
        barVel = 0f;
    ...
    {
        float dartChance = first ? 0.15f : Mathf.Lerp(0.18f, 0.48f, size) * stamina;
        if (Random.value < dartChance)
        {
            float away = fishY + (Random.value < 0.5f ? -1f : 1f) * Mathf.Lerp(0.22f, 0.55f, size);
            fishTarget = Mathf.Clamp01(away + Random.Range(-0.08f, 0.08f));
            dartSpeed = Mathf.Lerp(1.35f, 2.6f, size) * Mathf.Lerp(0.55f, 1f, stamina);
    ...
    void TickProgress(float dt)
    {
        ...
        float fill = Mathf.Lerp(0.24f, 0.11f, size);
        float drain = Mathf.Lerp(0.13f, 0.23f, size);
